...
- **Обычные формы `v8unpack` получили полное покрытие обработчиков снимка
  802.** Индекс v21 применяет закрытый реестр 544 структурных классов и 58
  событий, индексирует 53 328 форменных и элементных привязок без пропусков
  и лишних строк и безопасно отклоняет соседние позиции и версии. Команды
  обычных форм остаются `unsupported`.
- Оракул форм теперь сверяет полное мультимножество обработчиков, а
  одношаговый пробник строит 544 минимальные CF→JSON-дельты, фиксирует версии
  и хеши, проверяет зафиксированную матрицу и изолирует графический клиент в
  `xpra`; штатное открытие формы подтверждает `BeforeOpen` и `OnOpen`.
- Полная сборка переиспользует уже собранный контракт JSON-метаданных при
  индексации форм и обходит корректные и повреждённые привязки за один проход.
  На снимке 802 медиана трёх полных сборок составляет 171,47 с, прохода форм —
  19,86 с при максимуме 178 164 КиБ RSS.
- Проверка `form-inventory --index` теперь требует точного извлечения всех
  53 328 обычных обработчиков и нулевых `missed`, `misclassified`,
  `ambiguous` и `unexpected_index_rows`.
- **Основная структура метаданных `v8unpack` отделена от дополнительных
  граней.** Индекс v20 сохраняет 689/689 владельцев основной структуры,
  публикует отдельные facet-счётчики и ограниченную диагностику без потери
  полных сумм. Формы учитываются как покрытые только при наличии реальной
  проекции `form_elements`; неизвестные грани остаются заблокированными.
- **Обычные формы `v8unpack` больше не выглядят доказательно пустыми.**
  Индекс v19 извлекает подтверждённый форменный обработчик
  `0-27/ПриОткрытии`, отдельно публикует состояния полноты обработчиков,
  команд, атрибутов и элементов и сохраняет `unsupported` для недоказанных
  команд, событий и версий. `parse_form()` возвращает эти состояния для
  индексированных JSON-форм; CF/EDT не изменены.
- Оракул форм получил воспроизводимый живой инвентарь, закрытый контракт
  `OnOpen`, реестр неподдерживаемых проекций и строгую проверку доказательств.
  Полная сборка и update используют прежнюю атомарную замену без новой схемы
  SQLite.
