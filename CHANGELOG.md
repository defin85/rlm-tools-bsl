...
## [1.30.1+v8unpack.1] — 2026-07-30

Версия ответвления после слияния с upstream v1.30.1. Схема индекса ответвления
остаётся v21; после перехода с upstream требуется пересборка индекса.

### Исправлено

- Обычные формы `v8unpack` получили полное покрытие обработчиков снимка 802:
  закрытый реестр содержит 544 структурных класса и 58 событий.
- Индексируются 53 328 форменных и элементных привязок; проверка требует
  нулевых `missed`, `misclassified`, `ambiguous` и `unexpected_index_rows`.
- Основная структура метаданных отделена от дополнительных граней и публикует
  отдельные счётчики полноты.
- Удалён внутренний мост `symbol_provider`; внешние потребители должны читать
  SQLite или исходные `.bsl` самостоятельно.
