...
Если каталог исходников находится внутри git-рабочего дерева, автоматически подключается движок `git grep` как **бэкенд полнотекстового поиска** (хелпер `git_search`). Это закрывает системную дыру: SQLite-индекс и FTS5 ищут только по **именам** (методов, объектов, синонимов), а `git grep` ищет **любую подстроку по всем обычным файлам выбранного корня**, включая отслеживаемые, неотслеживаемые и исключённые через `.gitignore`, а также сырой `.xml`/`.mdo` (формы, права ролей, СКД, ConfigDumpInfo), тела процедур, строковые литералы и тексты запросов. Каталоги `.git` исключаются, символьные ссылки на внешние каталоги не обходятся. Ищется текущее состояние на диске; схема индекса не меняется и перестройка не требуется. Учтите: исключённые секреты и результаты сборки внутри выбранного корня тоже входят в поиск. Ограничивайте область через `path`, `file_types` и `exclude_path`. Требуется установленный `git`: в Docker-образе он уже установлен; на хосте Windows/Linux нужен `git` в `PATH`.
...
Формат исходников определяется автоматически. Адаптер `v8unpack` выбирается по
закрытому реестру пар: `obj_version=802` допускает маркер производителя
`v8unpack=1.2.9`, а `obj_version=803` — `v8unpack=1.2.6`. Версия объекта
выбирает позиционный контракт, версия производителя только разрешает его
применение. Обе фактические версии публикуются раздельно в
`v8unpack_metadata_object_version` и
`v8unpack_metadata_producer_version`; прежний
`v8unpack_metadata_version` остаётся псевдонимом версии производителя.

Индексатор читает только корневой файл и пары `<Kind>.json` /
`<Kind>.id.json` из зафиксированной матрицы. Формы, макеты, `*.elem.json` и
прочие JSON-файлы не индексируются. Структурно поддерживаются справочники,
документы, регистры сведений, накопления, бухгалтерии и планы видов
характеристик; остальные известные семейства участвуют только в идентификации
объектов и типов. Неизвестные версии, позиции, формы и UUID не подбираются:
адаптер возвращает `unsupported` или `partial` с ограниченной диагностикой.
Новая версия добавляется только после нулевого парного сравнения XML↔JSON из
одного CF и повышения `BUILDER_VERSION`. `--no-metadata` выставляет состояние
`disabled`, `--no-synonyms` независимо отключает синонимы.

После обновления с версии индекса ниже 17 выполните `index update`: команда
автоматически сделает полную перестройку.

#### Матрицы метаданных v8unpack 1.2.9/802 и 1.2.6/803

Матрица идентичности индексирует обязательную пару `<Kind>.json` /
`<Kind>.id.json` для следующих семейств:

`Catalog`, `Document`, `Enum`, `InformationRegister`, `AccumulationRegister`,
`AccountingRegister`, `CalculationRegister`, `ChartOfAccounts`,
`ChartOfCharacteristicType`, `ChartOfCalculationTypes`, `ExchangePlan`,
`BusinessProcess`, `Task`, `Constant`, `DefinedType`, `Subsystem`, `Role`,
`FunctionalOption`, `EventSubscription`, `ScheduledJob`, `CommonModule`,
`CommonForm`, `CommonCommand`, `Report`, `DataProcessor`, `DocumentJournal`.

У `CommonForm` поле `obj_version=13` является локальной версией формы: оно
подтверждено реальными парами 802 и 803 и не считается смешением корневых
версий. Настоящее смешение владельца 802↔803 по-прежнему блокирует все его
производные строки с диагностикой `unsupported_header_shape`.

Структурная матрица разбирает вложенные элементы только для шести семейств:

| Семейство | Поддерживаемые элементы |
|---|---|
| `Catalog` | реквизиты, табличные части и их реквизиты |
| `Document` | реквизиты, табличные части и их реквизиты |
| `InformationRegister` | измерения, ресурсы, реквизиты |
| `AccumulationRegister` | измерения, ресурсы, реквизиты |
| `AccountingRegister` | измерения, ресурсы, реквизиты |
| `ChartOfCharacteristicType` | реквизиты, табличные части и их реквизиты |

Ссылочные типы разрешаются через подтверждённые позиции `GeneratedType/TypeId`.
Неоднозначный или неизвестный UUID оставляет тип всей составной строки пустым и
не создаёт ложную обратную ссылку. Известные исключённые грани `header`
агрегируются как `unsupported_header_facet`; поэтому реальная конфигурация может
иметь состояние `partial`, даже когда все поддерживаемые строки успешно
проиндексированы.

Для `803` динамическая форма `Characteristic.*`, соответствующая XML
`v8:TypeSet`, сохраняется как пустой список типов `[]` и не создаёт ложную
ссылку. Доказанные отличия позиций `GeneratedType/TypeId` хранятся как малая
дельта к общей матрице 802/803: у `AccountingRegister` это чётные позиции
`2, 4, ..., 14`. Контракт встроенных UUID также версионный: для `803`
разрешены только три реально покрытых значения — `ValueStorage`, `UUID` и
универсальный компонент составного типа
`280f5f0e-9c8a-49cc-bf6d-4d296cc17a63`; остальные подтверждены только для
`802`.

Парный XML↔JSON-эталон создаётся репозиторным сравнителем:

```bash
python3 -m rlm_tools_bsl.v8unpack_oracle compare \
  --xml-root <xml> --json-root <json> --cf <config.cf> \
  --platform-version 8.3.27.1989 --v8unpack <исполняемый-файл> \
  --manifest <pair.manifest.json> --report <pair.report.json> \
  --command "<команда получения XML>" --command "<команда получения JSON>"
python3 -m rlm_tools_bsl.v8unpack_oracle manifest verify <pair.manifest.json> [...]
```

Сравнитель сохраняет кратность строк, хеши пяти канонических проекций и точных
наборов прочитанных XML/JSON-файлов. `--verify-existing` повторяет сравнение по
живым корням и отклоняет любое отличие от сохранённых манифеста и отчёта.
Совокупная проверка манифестов требует полного покрытия разрешённых позиций
генерируемых типов, встроенных UUID и структурной матрицы 803.

Набор `803` дополнен независимыми парами с реальными объектами БП:
`AccountingRegister`, `ChartOfCalculationTypes`, регистрами с реквизитами и
`ChartOfCharacteristicType` с реквизитами табличной части. Полная структурная
пара содержит 185 объектов, 191 поддерживаемый реквизит, 67 ссылок, 419
идентификаторов типов и 40 синонимов; все пять проекций имеют нулевую дельту.

Проверочный эталон получен из одного CF: этот же файл распакован `v8unpack
1.2.9` с `obj_version=802`, загружен во временную файловую базу и выгружен в
XML Конфигуратором 8.3.27. Нормализованные множества совпали без различий:
11700 атрибутов, 689 синонимов и 7634 ссылки. UUID совпали для 2428 объектов,
а `GeneratedType/TypeId` — для 6779 строк. Эталон содержит все шесть
структурных семейств, 4462 реквизита табличных частей и 358 составных типов.
При сравнении с XML русские синонимы очищаются от завершающих пробелов и
удвоенные кавычки приводятся к одной кавычке.

При обновлении весь JSON-слой заменяется атомарно до обработки BSL-дельты.
Снимок белого списка, состояние восстановления корня и диагностика сохраняются
в `index_meta` и публикуются в сведениях об индексе; для отключённого разбора,
CF/EDT и восстановления после повреждённого маркера снимок равен `null`. Этот
полный JSON-проход не означает повторную индексацию BSL.

Три чистые сборки зафиксированной выгрузки `obj_version=803` (1107 файлов,
831 JSON-файл, 129 BSL-файлов, 182 объекта метаданных) дали 0,56 / 0,53 /
0,54 секунды и 38260 / 38480 / 38380 КиБ максимального RSS. Размер каждого
индекса — 1269760 байт. Сборки выполнены 24 июля 2026 года через
`/usr/bin/time -v` на Linux 7.0.0, Intel Core i5-12400F, Python 3.12.13, с
новым `RLM_INDEX_DIR`, `build_calls=False` и `build_fts=False`. Медиана 0,54
секунды и максимальный RSS 38480 КиБ ниже приёмочных пределов 120 секунд и
524288 КиБ; показатели зависят от диска и состава конфигурации.

Малая выборка из реальных дескрипторов шести структурных семейств
(6 объектов, 41 поддерживаемый атрибут, 20 событий исключённых граней) дала
0,30 / 0,30 / 0,31 секунды, 41024–41100 КиБ RSS и индекс 446464 байта.
...
Индекс хранится в SQLite-базе `bsl_index.db`. В версии 17 используются внутренние
таблицы `metadata_objects` и `metadata_type_ids` для идентичности объектов
`v8unpack` и разрешения ссылочных типов.
...
| key                     | Описание                                                | Пример                             |
| ----------------------- | ------------------------------------------------------- | ---------------------------------- |
| `version`               | Версия схемы                                            | `17`                               |
| `builder_version`       | Версия построителя                                      | `17`                               |
...
| `source_format`         | Формат исходников                                       | `cf`, `edt` или `v8unpack`         |
| `v8unpack_metadata_status` | Полнота JSON-слоя                                    | `complete`, `partial`, `unsupported`, `disabled`, `not_applicable` |
| `v8unpack_metadata_producer_version` | Маркер производителя JSON                  | `1.2.6`                            |
| `v8unpack_metadata_object_version` | Версия позиционного контракта                    | `803`                              |
| `v8unpack_metadata_version` | Псевдоним версии производителя для совместимости    | `1.2.6`                            |
| `v8unpack_metadata_structural_total` | Владельцы обязательной основной структуры       | `689`                              |
| `v8unpack_metadata_structural_indexed` | Владельцы с полностью построенной основной структурой | `689`                         |
| `v8unpack_metadata_structural_failed` | Владельцы с ошибкой основной структуры           | `0`                                |
| `v8unpack_metadata_facet_total` | Обнаруженные дополнительные грани                      | `1787`                             |
| `v8unpack_metadata_facet_supported` | Грани, уже покрытые отдельной проекцией              | `1388`                             |
| `v8unpack_metadata_facet_unsupported` | Измеряемые, но ещё не покрытые грани                | `399`                              |
| `v8unpack_metadata_facets_json` | Детерминированная разбивка граней по семейству, позиции, UUID и классу | `[...]` |
| `v8unpack_metadata_unsupported_count` | Полное число неподдержанных событий, включая диагностические ошибки | `399` |
| `v8unpack_metadata_diagnostic_groups_total` | Полное число агрегированных групп до ограничения выдачи | `13` |
...
Показатели основной структуры и дополнительных граней независимы.
`structural_indexed == structural_total` означает только полноту существующих
структурных таблиц. Известные формы, макеты и команды учитываются как грани:
формы считаются покрытыми, когда их UUID присутствуют в отдельной проекции
`form_elements`; макеты и команды остаются измеряемыми до появления
специфицированной пользовательской проекции. Неизвестная грань всегда
`blocked`. Пока есть непокрытые или заблокированные грани, общий
`v8unpack_metadata_status` остаётся `partial`.
Диагностический JSON содержит не более 50 групп и 5 примеров на группу;
`unsupported_count` и `diagnostic_groups_total` при этом не усекаются.

Новая грань добавляется только закрытой записью
`(семейство, позиция header, UUID)` после сравнения XML и JSON одной
конфигурации. Совпадение UUID или частоты без парного доказательства не
разрешает менять её класс.

Если точный CF рабочего JSON-дерева невосстановим, допустим только явно
согласованный доказательный мост: повторный `/DumpCfg` подтверждает хеш
сохранённого парного CF, парный XML↔JSON-снимок доказывает семантику закрытой
формы, а отдельный хеш активного дерева и фактический UUID целевой формы —
применимость. Такой мост не доказывает полное равенство двух JSON-деревьев.
...
подтверждённому контракту. Индекс v21 дополнительно оценивает для каждой формы
...
Для обычных форм снимка 802 закрытый реестр содержит 544 точных класса
«локальная версия + версия элементов + позиционный путь + область + тип
элемента + сырое событие» и 58 канонических событий. Форменные,
дополнительные и элементные обработчики индексируются с точными `scope`,
`element_name`, `element_type`, `event`, `handler` и `data_path`. Известный
код события на соседней позиции или в другой версии не принимается.
Команды обычных форм по-прежнему не разбираются:
`projections.commands="unsupported"`. Поэтому общий статус структуры
остаётся `partial`, хотя у роли `handlers` живого снимка
