...
Актуально для **v1.29.2** (`BUILDER_VERSION = 20`; после обновления с предыдущей версии требуется `index update`).
...
- **`format_detector.py`** — определение CF/EDT/v8unpack (`detect_format`, `SourceFormat`, `FormatInfo`), парсинг путей BSL-файлов (`parse_bsl_path`, `BslFileInfo`), `METADATA_CATEGORIES`, `MODULE_TYPE_MAP`. JSON-метаданные v8unpack не разбираются. → _(нет внутренних зависимостей)_
